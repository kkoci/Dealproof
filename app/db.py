...
async def create_dev_credentials_table() -> None:
    """Create the dev_credentials table if it does not exist."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """
...
async def create_dev_cred_record(
    credential_id: str,
    ...
    """Insert a new dev credential record in 'pending' status."""
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            """
            INSERT INTO dev_credentials
              (credential_id, developer_handle, repo_corpus_root, commit_count,
               metrics_json, status)
            VALUES (?, ?, ?, ?, ?, 'pending')
            """,
            (
...
async def update_dev_cred_evaluated(
    credential_id: str,
    credential: dict,
    tee_quote: str | None,
) -> None:
...
async def get_dev_cred(credential_id: str) -> dict | None:
    """Fetch a dev credential record by ID. Returns dict or None."""
    async with aiosqlite.connect(DB_PATH) as db:
        async with db.execute(
            """
