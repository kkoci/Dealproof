...
def deduplicate_commits(commits: list[dict], already_seen: set[str] | None = None) -> list[dict]:
    """
    Removes commits whose SHA already appears earlier in `commits` or in `already_seen`.

    A commit reachable from multiple branches (e.g. already merged into main and still
    visible on the feature branch it came from) must only be counted once — otherwise
    metrics like total_commits/active_months double-count real work rather than adding
    genuinely missing history. Order-preserving: first occurrence wins.
    """
