...
def record_and_check_spend(session_id: str, cap: int = SPEND_CAP_PER_SESSION) -> None:
    ...
    `cap` defaults to SPEND_CAP_PER_SESSION (Offer Check's own limit) so every
    existing call site is unaffected; callers with a different per-call-site
    cap (e.g. Agent Rail's mediator.py, which passes its own lower limit) can
    override it without duplicating the counting/state machinery below."""
    count = _CALL_COUNTS.get(session_id, 0) + 1
    _CALL_COUNTS[session_id] = count
    if count > cap:
        raise SpendCapExceeded(f"session {session_id} exceeded the {cap}-call spend cap")
